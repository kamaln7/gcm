...
public class AddAttractionAndUpdateMapImageCommand implements Command {
    ...
    public Output runOnServer(Request request, Server server, ConnectionToClient client) throws Exception{
        ...
        Files.write(Paths.get(imageName), input.new_map_image);

        Map mapToUpdate = Map.findByTitleAndVersion(input.map_title,input.map_version);
        //delete the old file from the server
        File file = new File(mapToUpdate.getImg());
        file.delete();


        mapToUpdate.updateImage(imageName,input.map_version,input.map_title);

        City city = City.findById(mapToUpdate.getCityId());

        Attraction attraction = new Attraction(input.attraction_name,city.getName(),input.attraction_type,input.getAttraction_location);
        attraction.insert();
